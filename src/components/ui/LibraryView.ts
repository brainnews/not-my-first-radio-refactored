...
import { getAnimatedEqualizerIcon, getBitrateIcon, createStationInitialsImage } from '@/utils/icons';
import { autoListGenerator, AutoGeneratedList } from '@/services/lists/AutoListGenerator';
...
export class LibraryView {
  ...
  private autoGeneratedLists: AutoGeneratedList[] = [];
  ...
  /**
   * Load stations from storage and generate auto lists
   */
  private loadStations(): void {
    this.stations = getStorageItem(StorageKeys.STATIONS, []);
    this.generateAutoLists();
  }

  /**
   * Generate auto-generated lists based on current stations
   */
  private generateAutoLists(): void {
    this.autoGeneratedLists = autoListGenerator.generateLists(this.stations);
  }
  ...
  private renderStationsContent(container: HTMLElement): void {
    ...
    // Auto-generated lists section
    if (this.autoGeneratedLists.length > 0) {
      this.renderAutoGeneratedLists(container);
    }
  ...
  /**
   * Render auto-generated lists section
   */
  private renderAutoGeneratedLists(container: HTMLElement): void {
    // Render each auto-generated list as a simple section
    this.autoGeneratedLists.forEach(list => {
      const listSection = this.createAutoListSection(list);
      container.appendChild(listSection);
    });
  }

  /**
   * Create an auto-generated list section
   */
  private createAutoListSection(list: AutoGeneratedList): HTMLElement {
    const section = createElement('div', { 
      className: 'library-auto-list-section'
    });
    section.dataset.listId = list.id;
    
    // Header with title and dismiss button
    const header = createElement('div', { className: 'library-auto-list-header' });
    
    const title = createElement('h3', { className: 'library-auto-list-title' }, [list.title]);
    
    // Dismiss button
    const dismissButton = createElement('button', {
      className: 'library-auto-list-dismiss',
      title: 'Dismiss this list'
    }) as HTMLButtonElement;
    dismissButton.setAttribute('aria-label', 'Dismiss this list');
    dismissButton.innerHTML = '<span class="material-symbols-rounded">close</span>';
    dismissButton.addEventListener('click', (e) => {
      e.stopPropagation();
      this.dismissAutoList(list.id);
    });
    
    header.appendChild(title);
    header.appendChild(dismissButton);
    
    // Horizontal scrolling container for stations
    const scrollContainer = createElement('div', { className: 'library-auto-list-scroll' });
    const stationsGrid = createElement('div', { className: 'library-auto-list-stations' });
    
    // Show all stations
    list.stations.forEach(station => {
      const stationCard = this.createSimpleStationCard(station);
      stationsGrid.appendChild(stationCard);
    });
    
    scrollContainer.appendChild(stationsGrid);
  ...
  /**
   * Create a simple station card for auto-generated lists
   */
  private createSimpleStationCard(station: LocalStation): HTMLElement {
    const card = createElement('div', { className: 'library-auto-station-card' });
    
    // Clean the station name for display
    const cleanedName = autoListGenerator.cleanStationName(station.name);
    
    // Station avatar/logo
    const avatar = createElement('div', { className: 'library-station-avatar' });
    if (station.favicon) {
      ...
      img.onerror = () => {
        // Replace failed favicon with generated initials favicon
        img.remove();
        const initialsImg = createStationInitialsImage(cleanedName, 120);
        avatar.appendChild(initialsImg);
      };
      avatar.appendChild(img);
    } else {
      // No favicon provided, use generated initials favicon
      const initialsImg = createStationInitialsImage(cleanedName, 120);
      avatar.appendChild(initialsImg);
    }
    
    // Station name container
    const stationNameContainer = createElement('div', { className: 'library-auto-station-name-container' });

    // Station name (cleaned)
    const name = createElement('div', { className: 'library-auto-station-name' }, [cleanedName]);
    
    // Click handler to play the station
    card.addEventListener('click', () => {
      eventManager.emit('station:play-request', station);
    });
    
    card.appendChild(avatar);
    card.appendChild(stationNameContainer);
    stationNameContainer.appendChild(name);
   ...
   */
  private dismissAutoList(listId: string): void {
    autoListGenerator.dismissList(listId);
    
    // Remove the list from current display
    const section = this.container.querySelector(`[data-list-id="${listId}"]`);
    if (section) {
      section.remove();
    }
    ...
    // Refresh the lists
    this.generateAutoLists();
    this.renderAutoGeneratedLists(this.container.querySelector('.library-content') as HTMLElement);
  }
  ...
  /**
   * Handle auto-lists generation request
   */
  private handleAutoListsGenerate = (): void => {
    this.generateAutoLists();
    this.render();
    eventManager.emit('auto-lists:generated', this.autoGeneratedLists);
  };
  ...
  private setupEventListeners(): void {
    ...
    // Listen for auto-lists events
    eventManager.on('auto-lists:generate', this.handleAutoListsGenerate);
  }
  ...
  destroy(): void {
    ...
    eventManager.off('auto-lists:generate', this.handleAutoListsGenerate);
